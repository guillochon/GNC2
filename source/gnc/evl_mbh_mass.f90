...
subroutine get_mbh_increase(dmbh_tot)
    ...
    if(rid.eq.0)then
        ...
        write(unit=*,fmt="(20A12)") "TD_bound","TD_unbound","LC(star)","LC(SBH)","EMAX(SBH)","EMRI(SBH)"
        write(unit=*,fmt="(1P20E12.3)") mbh_mmg%td_direct(1), mbh_mmg%td_disc(1), mbh_mmg%lc_direct(1:2), &
        mbh_mmg%emax_direct(2), mbh_mmg%emri(2)
    end if
    if(rid.eq.0)then
